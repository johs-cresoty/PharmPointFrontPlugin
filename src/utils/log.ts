...
/**
 * 보관 줄 수. 거래 1건에 6~10줄이라 최근 거래 오십~여든 건 남짓 담긴다.
 *
 * 진단 보내기는 줄 수와 무관하게 Sentry 이벤트 1건이라, 늘려도 무료 한도(월 5,000건)를
 * 더 쓰지 않는다. 500줄이면 약 30KB 로 이벤트 크기 상한에도 한참 못 미친다.
 *
 * 다만 시간이 아니라 줄 수로 밀려나는 구조다. 거래가 많은 약국은 500줄이 하루도
 * 안 될 수 있으니, 문의를 받으면 거래가 더 쌓이기 전에 눌러야 한다.
 */
const LINK_LOG_MAX = 500;
