/**
 * SerialTransport — Toss Front SDK Serial API 래퍼.
 *
 * 역할: 단말기(TRM) 바이너리 전문 송수신.
 *   - sdk.serial.open    → 시리얼 포트 오픈 (baudRate 만 파라미터 노출됨)
 ...
 *   완성 못한 잔여 바이트는 다음 chunk 와 합쳐 재시도.
 */
...
export function createSerialTransport({ onFrame, onError }: SerialTransportHandlers): SerialTransport {
  ...
  function tryExtractFrames(): void {
    while (state.buffer.length > 0) {
      ...
      if (stxIdx < 0) {
        // STX 없으면 단일 제어 바이트 (ACK/NAK 등) 가능 → 버퍼 비움
        state.buffer = new Uint8Array(0);
        return;
      }
      if (stxIdx > 0) state.buffer = state.buffer.slice(stxIdx);

      // 2) 길이 헤더 (STX 다음 ~ 4자리 ASCII digit)
      if (state.buffer.length < 8) return; // STX + XX + len(4) 최소
      let pos = 0;
      let found = -1;
